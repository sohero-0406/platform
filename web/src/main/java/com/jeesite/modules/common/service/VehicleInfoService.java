...
public class VehicleInfoService extends CrudService<VehicleInfoDao, VehicleInfo> {
     ...
     * @return
     */
    public VehicleInfo getByEntity(VehicleInfo vehicleInfo) {
        return dao.getByEntity(vehicleInfo);
    }
...
//		}
    ...
    public Map<String, Map<String, List<VehicleInfo>>> findAllForAppraisal(String chexi) {
        List<VehicleInfo> list = dao.findAllForAppraisal(chexi);
        for (VehicleInfo vehicleInfo : list) {
            String chexingmingcheng = vehicleInfo.getChexi() + "-" + vehicleInfo.getNiankuan() + "-";
            String fadongji = vehicleInfo.getFadongji();
            if (StringUtils.isNotBlank(fadongji) && !"未知".equals(fadongji)) {
                chexingmingcheng += fadongji.split(" ")[0];
            }
            vehicleInfo.setChexingmingcheng(chexingmingcheng);
            vehicleInfo.setChexi(null);
            vehicleInfo.setNiankuan(null);
            vehicleInfo.setFadongji(null);
        }
        Map<String, Map<String, List<VehicleInfo>>> collect = list.stream().collect(Collectors.groupingBy(VehicleInfo::getShouzimu, Collectors.groupingBy(VehicleInfo::getPinpai)));
        return collect;
    }

    public FindDetailForAppraisalRespVO findDetailForAppraisal(String id) {
        VehicleInfo vehicleInfo = dao.findDetailForAppraisal(id);
        FindDetailForAppraisalRespVO respVO = new FindDetailForAppraisalRespVO();
        BeanUtils.copyProperties(vehicleInfo, respVO);
        CommonVehicleImage commonVehicleImage = new CommonVehicleImage();
        commonVehicleImage.setVehicleId(id);
        List<String> imageList = new ArrayList<>();
        List<CommonVehicleImage> list = commonVehicleImageService.findList(commonVehicleImage);
        for (CommonVehicleImage vehicleImage : list) {
            imageList.add(vehicleImage.getImageName());
        }
        respVO.setImageList(imageList);
        return respVO;
    }
}